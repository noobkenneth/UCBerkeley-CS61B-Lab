...
public class DLList<T extends Comparable<T>> {
    ...
    private void insertionSortHelper(T item) {
        ...
        while (x != sentinel && item.compareTo(x.item) < 0) {
            x = x.prev;
        }
    ...
    public DLList<T> mergeSort() {
        ...
        while (count < size && x != sentinel) {
            if (count < size / 2) {
                oneHalf.addLast(x.item);
            } else {
                otherHalf.addLast(x.item);
            }
            count += 1;
            x = x.next;
        }
    ...
    public DLList<T> quicksort() {
        ...
        int count = 0;
        while (count < size && x != sentinel) {
            if (pivot.compareTo(x.item) == 0) {
                equalElements.addLast(x.item);
            } else if (pivot.compareTo(x.item) > 0) {
                smallElements.addLast(x.item);
            } else {
                largeElements.addLast(x.item);
            }
            count += 1;
            x = x.next;
        }
       ...
       // equalElements = equalElements.quicksort();
        largeElements = largeElements.quicksort();

        return smallElements.merge(equalElements).merge(largeElements);
