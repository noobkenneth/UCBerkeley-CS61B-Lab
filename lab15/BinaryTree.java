public class BinaryTree<T> {
    ...
    public static int heightHelper(TreeNode t) {
        ...
        if (t.left == null && t.right == null) {
            ...
            int tallestHeight = 1;
            if (t.right != null) {
                TreeNode rightSide = t.right;
                tallestHeight = Math.max(tallestHeight, heightHelper(rightSide));
            }
            if (t.left != null) {
                TreeNode leftSide = t.left;
                tallestHeight = Math.max(tallestHeight, heightHelper(leftSide));
            }
            tallestHeight += 1;
    ...
    public static TreeNode binaryHelper(int N) {
        if (N == 0) {
        ...
        TreeNode t = new TreeNode<Integer>(getFibNumber(N));
        t.left = binaryHelper(N - 1);
        t.right = binaryHelper(N - 2);
        return t;
    }

    /* Returns a BinaryTree representing the Fibonacci calculation for N. */
    public static BinaryTree<Integer> fibTree(int N) {
        BinaryTree<Integer> result = new BinaryTree<Integer>(binaryHelper(N));
        return result;
    }
